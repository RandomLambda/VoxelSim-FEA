...
* ``_inside_mask_trimesh`` - uses ``trimesh`` + ``rtree`` for the broad-phase.
  ...
  ``blender_manifest.toml`` (the Extensions Platform requires a feature not
  depend on a library the extension doesn't ship itself). Measured against
  the numpy path: trimesh's fixed overhead from building the Trimesh object
  + rtree index makes it lose clearly below a few hundred triangles, and win
  clearly from roughly a thousand triangles up, with the numpy path becoming
  impractically slow (or hitting the memory blow-up described above) well
  before that. Kept as a separate function so the tiny/degenerate-mesh case
  still gets the faster numpy path, and so there is a fallback if a platform
  has no matching wheel.
...
# below it numpy wins outright, since trimesh's fixed setup cost (building
# the Trimesh object + rtree index) dominates at small triangle counts.
...
def inside_mask(verts, faces, points, direction=None, prefer_trimesh=True,
    ...
    justify their fixed overhead (see _TRIMESH_TRI_THRESHOLD); otherwise the
