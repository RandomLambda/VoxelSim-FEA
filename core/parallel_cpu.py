...
Implementation notes:
  ...
  * Each worker pins its own BLAS thread pool (OpenBLAS/MKL, via
    threadpoolctl) to 1 thread. Without this, ``contrib = ue @ KE.T`` --
    small, but still a BLAS call -- lets every worker's numpy spin up its
    own default-sized thread pool (typically = all logical cores), so
    N_workers processes each try to claim every core at once: pure
    oversubscription on top of the N-way process split that is supposed to
    be the parallelism, and it doesn't fail loudly, it just thrashes (more
    workers or bigger chunks make it worse, not better). The calling
    process's own BLAS thread count is intentionally left alone here (its
    O(ndof) vector work is cheap and does not run inside these workers) --
    see compute_plan.choose()'s multi_cpu branch for where that side is
    capped.
"""
...
def _limit_worker_blas_threads():
    """Pin this worker's BLAS thread pool to 1 (see module docstring).
    Best-effort: threadpoolctl is a bundled wheel, but a worker that
    somehow can't import it should still compute correctly, just slower.
    """
    try:
